...
#
#     This script is retributed under the same conditions as part of my hobby project WWW Security Assistant (https://github.com/metalevel-tech/wwwsas)
...
#         1) Modify line 113 to: my $PROTECT_EXEC = "/bin/echo %s";
#         2) Then run from CLI: cat /path-to-modsecurity2-log-dir/modsec_guardian.log | ./httpd-guardian.pl
#        
#     To configure ModSecurity to work with this script, without using 'wwwsas.sh' (Iptables chain WWWSAS is required):
#         1) Modify: my $PROTECT_EXEC = "/sbin/iptables -A WWWSAS -s %s -j DROP && /bin/echo /sbin/iptables -D WWWSAS -s %s -j DROP | at now + 5 minutes";
#         2) Edit '/etc/modsecurity/modsecurity.conf' and pipe the Guardian log to the script: SecGuardianLog "|/path-to/httpd-guardian.pl"
#
#     To configure this script to work with 'wwwsas.sh':
#
...
my $COPY_LOG = "/var/log/apache2_mod_security/modsec_httpd-guardian.pl.log";
