...
# Name:    modules/modsecurity-whitelist-rule-generator.sh
# Summary: The tool is able to parse the ModSecurity's audit log file (modsec_audit.log) and
#          generate whitelisting rules, based on the REQUEST_URI. The necessary configuration data must be provided within wwwsas.conf.
#
...
#      When the value is 'active-rules' (used by 'wwwsas.sh'), it just will output information about the rules involved in the thread.
...
LOG_FILE="${WWW_SAS_TMP}/modsec_audit.cat.log"
TMP_FILE="${WWW_SAS_TMP}/modsec_audit.cat.tmp"
