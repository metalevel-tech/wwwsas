...
#          The script is designed to be run via CRON Job as well as SHELL Command.
...
# - Default mode, that will outut the result in CLI: '/path/to/flood-detector.bash' or 'flood-detector'
# - AutoMode, that should be used in Root`s Crontab: '* * * * * "/path/to/flood-detector.bash" 'AutoMode' >> "/path/to/www-security-assistant.exec.log" 2>&1'
...
TMP_FILE="/tmp/www-security-assistant-flood-detector.tmp"
				...
				do
					...
					then
						...
						then
							...
							## Output a log header
							printf '\n\n*****\nSECURITY LOG from %s on %s : flood-detector.bash >>' "$TIME" "$DATE" >> "$WWW_SAS_EXEC_LOG" 2>&1
							# Call WWW Security Assistant Script
							exec "$WWW_SAS_EXEC" "$ATTACKING_IP" 'FloodDetector' "$ATTACK_INFO" >> "$WWW_SAS_EXEC_LOG" 2>&1 &
						else
