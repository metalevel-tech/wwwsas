...
19. [HTTPS and Common Apache Security Tips](#https-and-common-apache-security-tips)
...
The script have a kind of integration with [`www.abuseipdb.com`](https://www.abuseipdb.com). Is can curl data from their API (further probably will have automatic report feature), and provide it by the emails. If you want to use this feature, you should register on [`www.abuseipdb.com`](https://www.abuseipdb.com) and provide your API key within the configuration file - variable `$AbuseIPDB_APIKEY`.
...
4. The next step is **Setup (update) WWW Security Assistant Configuration**. Here the installation script will create local `.conf` files, based on the `.example` files. You will be informed about each file creation and will be asked to edit it.
...
5. **Create shell commands**. The setup script will make symbolic links for few of our scripts to `/usr/local/bin` thus they be accessible as shell commands. Within the emails that will be sent by the main script, you will receive instructions how to use most of these commands. The created commands are:
   ...
   * `wwwsas` that is a symbolic link to [`www-security-assistant.bash`](#www-security-assistant-script-wwwsas--iptables),
   * `wwwsas-fd` that is a symbolic link to [`flood-detector.bash`](#wwwsas-flood-detector--wwwsas--iptables),
   * `wwwsas-mswlrg` that is a symbolic link to [`modsecurity-whitelist-rule-generator.bash`](#wwwsas-whitelist-rule-generator--modsecurity-rules-whitelisting),
   * `wwwsas-iptables-save` that is a symbolic link to [`ipset-save.sh`](#iptables--basic-configuration--save-and-restore),
   * `wwwsas-iptables-restore` that is a symbolic link to [`ipset-restore.sh`](#iptables--basic-configuration--save-and-restore).
...
7. In this place the script will **Create 'crontab' entry for 'flood-detector.bash'**. The *cronjob* will be executed on each minute (see the relevant section about this script below). You can skip this step, but `flood-detector.bash` doesn't consume much server resources and, you will see, time-to-time it does nice work.
...
* Create log file and Restart the Apache server:
  ...
  ````bash
  sudo touch /etc/www-security-assistant/www-security-assistant.exec.log
  sudo chown www-data /etc/www-security-assistant/www-security-assistant.exec.log
...
The automatic mode should be used when the script is called from `crontab` (`sudo crontab -e`). In this mode when a flood attack is detected the script will execute `www-security-assistant.bash`. For this mode we must provide `AutoMode` as value of the first positional parameter:

````bash
* * * * * "/etc/www-security-assistant/flood-detector.bash" 'AutoMode' >> "/etc/www-security-assistant/www-security-assistant.exec.log" 2>&1
````

During the installation process the necessary steps, according this feature, will be applied by the `SETUP` script.
