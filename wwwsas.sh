...
# Remove $IP fromthe ACCEPT (WHITE) List, syntax: wwwsas.sh <IP> --ACCEPT-REMOVE
# Ref.: https://backreference.org/2010/02/20/using-different-delimiters-in-sed/#comment-25216
if [[ $AGENT == "--ACCEPT-REMOVE" ]]
then

    # Remove the entry from '/etc/wwwsas/logs/white-list.log' 
    ...
    # Remove the entry from '/etc/wwwsas/confs/modsec.ip.white-list.conf'
...
# Remove $IP from the DROP (BAN) List, syntax: wwwsas.sh <IP> --CLEAR 'log notes'"
elif [[ $AGENT == "--CLEAR" ]]
then
...
then
    ...
    then
        ...
        # Backup ModEvasive lock file in case it exists
        find "$MOD_EVASIVE_LOG_DIR" -maxdepth 1 -type f -exec mv {} "$MOD_EVASIVE_LOG_DIR_BAK" \;
...
# Add $IP to the DROP (BAN) List, syntax: wwwsas.sh <IP> --DROP 'log notes'"
elif [[ $AGENT == "--DROP" ]]
then
...
# Add $IP to the ACCEPT (WHITE) List, syntax: wwwsas.sh <IP> --ACCEPT 'log notes'
elif [[ $AGENT =~ "--ACCEPT" ]]
then

    # Add an entry to '/etc/wwwsas/logs/white-list.log'
    ...
    # Add an entry to '/etc/wwwsas/confs/modsec.ip.white-list.conf'
    ...
    # Add $IP to the ACCEPT (WHITE) List and add IPTables rule, syntax: wwwsas.sh <IP> --ACCEPT-CHAIN 'log notes'"
    ...
    then
        ...
        /sbin/iptables -L "$WWW_SAS_IPTBL_CHAIN" -w -n --line-numbers
        eval "$WWW_SAS_IPTABLES_SAVE"
        # Add an entry to '/etc/wwwsas/logs/white-list.log' - thi is a duplicate of the above record, but it is ok
...
then
    ...
    # Prepare the log notes, that comes from modules/modsecurity-assistant.sh and modules/flood-detector.sh
